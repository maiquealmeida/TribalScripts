...
// elemento tooltip
var $tooltip;
// objeto com as traduÃ§Ãµes da linguagem do usuÃ¡rio
var lang;
// historico de log
var logHistory = [];
// dados padrÃ£o do cache do script
var defaults = {
...
/** COMMAND_PLANNER - Envia ataques/apoios em horÃ¡rios e unidades prÃ©-definidas */
var COMMAND_PLANNER = function() {
	...
	// divisÃ³ria para cada um na tabela
	jQuery.each([
		...
		// loop em todas unidades da categoria em questÃ£o
		jQuery.each(this, function( i, unit ) {
			// verifica se a unidade existe no banco de configuraÃ§Ãµes pois
			// existem mundos que nÃ£o arqueiros e paladinos nÃ£o sÃ£o ativados
			if ( DATA.units[ unit ] ) {
	...
	// ao clicar no botÃ£o "Adicionar comando" Ã© chamada a funÃ§Ã£o ADD
	app.find( 'input[type=button]' ).click(function() {
		ADD.apply( window, PREPARE_ADD() );
	});
	
	// ao apertar a tecla "Enter" Ã© chamada a funÃ§Ã£o ADD
	...
	app.find( '.TWA_ALL' ).click(function() {
		...
		// caso a opÃ§Ã£o seja descelecionada, ativa o campo novavemte
		if ( !this.checked ) {
		...
		// caso a coordendas inserida nÃ£o bata com o padrÃ£o
	...
	/** COMMAND_PLANNER -> PREPARE_ADD - Ã‰ gerado os argumentos da funÃ§Ã£o ADD apartir das entradas com os dados do comando */
	 ...
	 * from {String} - coordenadas da aldeia de enviarÃ¡ o comando.
	 * to {String} - coordenadas da aldeia que receberÃ¡ o comando.
	 * time {Number} - horÃ¡rio que o comando serÃ¡ enviado.
	...
	function ADD( from, to, time, support, units ) {
		...
		// Ã© verificado se os valores passados ao comando estÃ£o em ordem
	...
	function UPDATE() {
		...
		// caso tenha algum comando na fila de espera Ã© mostrado a table com o titulo na janela
	 ...
	 * elem {Element|Null} - Elemento que serÃ¡ atualizado individualmente. Quando nulo, Ã© atualizado todos elementos da lista. */
	 ...
	 * index {Number|Boolean} - Index do comando que serÃ¡ removido. Caso seja true, remove todos os comandos atrasados. */
	...
	function CHECK_IN( no_confirm ) {
		...
		for ( i = 0; commands[i]; ) {
			...
			// caso o comando esteja atrasado e a confirmaÃ§Ã£o esteja ativada
			if ( !no_confirm && now - 5000 > cmd.time ) {
				// gera a confirmaÃ§Ã£o para o usuario com as opÃ§Ãµes de enviar os comandos ou cancela-los
				min = ( now - cmd.time ) / 1000 / 60;
				
				// mostra a confirmaÃ§Ã£o para o usuÃ¡rio
			...
			// com antecedencia e nÃ£o encher de requisiÃ§Ãµes ajax em cima
			...
			// caso seja maior a funÃ§Ã£o Ã© executada na hora causando erros
	 ...
	 * cmd {Object} - Objeto com os dados necessÃ¡rios para preparar o envio do comando. */
	function PREPARE_COMMAND( cmd ) {
		LOG( 'COMMAND_PLANNER->PREPARE_COMMAND()' );
		
		// coordenadas da aldeia que enviarÃ¡ o comando
		var from = cmd.from.join( '|' );
		// gera os dados do comando
		var data = {
			x: cmd.to[ 0 ],
			y: cmd.to[ 1 ],
			template_id: '' // para simulaÃ§Ã£o de formulario
		};
		
		// adiciona aos dados se o comando serÃ¡ um ataque ou apoio
		...
		GET_VILLAGE_UNITS(cmd.from.join( '|' ), function( units ) {
			...
			// ativadas para enviar todas, serÃ¡ substituida pela quantidade
			...
			// pega os dados da aldeia que enviarÃ¡ o comando
	...
	/** COMMAND_PLANNER -> SEND - Envia a requisiÃ§Ã£o ajax para realizar o envio do comando.
	 * cmd {Object} - Objeto do comando.
	 * data {Object} - Objeto com os dados necessÃ¡rios para realizar o comando.
	 * vid {Number} - ID da aldeia que realizarÃ¡ o comando. */
	function SEND( cmd, data, vid ) {
		LOG( 'COMMAND_PLANNER->SEND( ' + JSON.stringify( cmd ) + ' )' );
		
		// requisiÃ§Ã£o 1/2 para o envio do comando
		jQuery.post(URL( 'place&try=confirm', vid ), data, function( html ) {
			...
			// caso o elemento exista, Ã© mostrado o erro no LOG
			if ( error = error.text() ) {
...
/** AUTO_FARM - Envia ataques em coordenadas e unidades prÃ©-definidas em ciclos */
var AUTO_FARM = function() {
	...
	var autofarms = {};
	// cÃ³digo unico enviado em cada requisiÃ§Ã£o de comando
	var token = {};
	// elementos <th> da tabela com os icones das unidades
	var header = '';
	
	// carrega a praÃ§a de reuniÃ£o e obtem o token
	...
	app.find( '.TWA_ADDMODEL' ).click(function() {
		// nome que serÃ¡ dado ao modelo
		var mid = jQuery( '[name=TWA_MODELNAME]' ).val();
		
		// verifica se ja hÃ¡ algum modelo com o nome escolhido
		if ( models[ mid ] ) {
	...
	jQuery( document ).on('click', '.TWA_SAVE', function() {
		...
		for ( var i = 0; i < data.length; i++ ) {
			switch ( data[i].name ) {
				...
				break;
				case 'coords':
					// as coordenadas sÃ£o cortadas pelos espaÃ§os (Array)
		...
		if ( button.hasClass( 'paused' ) || this.className === 'TWA_START' ) {
			// verifica se estÃ¡ com as coordendas e unidades adicionadas ao modelo
			if ( autofarms[ mid ].check() ) {
				// inicia os ataques
				autofarms[ mid ].start();
				
				// altera as informaÃ§Ãµes do input
		...
		// envia uma confirmaÃ§Ã£o ao usuario para dar certeza de remover o modelo
	...
	function UPDATE() {
		...
		for ( mid in models ) {
			...
			if ( !autofarms[ mid ] ) {
				// Ã© gerado uma nova instancia do AutoFarm para o modelo em questÃ£o
		...
		// faz o loop em cada autofarm em execuÃ§Ã£o para deixar cada input
	...
	/** Autofarm
	 * mid {String} - Nome do modelo que serÃ¡ iniciado */
	...
	/** Autofarm.check - Verifica se hÃ¡ coordendas e unidades inseridas no modelo. */
	...
	/** Autofarm.prepare - Prepara os dados e obtem as informaÃ§Ãµes bÃ¡sicas para o ataque partir.
	 * coords {String} - Coordenada da aldeia que sofrerÃ¡ o ataque */
	Autofarm.prototype.prepare = function( coords ) {
		LOG( 'AUTO_FARM->PREPARE( ' + coords + ', ' + this.mid +  ' )' );
		
		// caso esteja pausado para a funÃ§Ã£o
		if ( this.paused ) {
			return this;
		}
		...
		var self = this;
		// array que guardarÃ¡ os nomes das unidades que seram enviadas todas da aldeia
		var getAll = [];
		var data = {
			x: coords[ 0 ],
			y: coords[ 1 ],
		...
		// adiciona o token aos dados de envio para simulaÃ§Ã£o de formulario
		jQuery.extend( data, token );
		// adiciona as unidades que seram enviadas nos dados
		jQuery.extend( data, self.data.units );
		
		// verifica as unidades que serÃ£o enviadas todas e joga no array getAll
		for ( var name in this.data.units ) {
		...
		// caso tenha alguma unidade que serÃ¡ enviada todas
		if ( getAll.length ) {
			...
			return GET_VILLAGE_UNITS(game_data.village.id, function( units ) {
				...
				// chama a Ãºltima etapa do envio
				self.send( data );
			});
		}
			...
			if ( error = error.text() ) {
				...
				GET_VILLAGE_UNITS(html, function( units ) {
					...
					for ( var name in self.data.units ) {
						...
						// caso a aldeia nÃ£o tenha as unidades exigidas pelo modelo,
					...
					if ( !jQuery.isEmptyObject( needUnits ) ) {
						...
						GET_COMMAND_TIME(game_data.village.id, function( time ) {
							// caso nÃ£o tenha nenhum comando em andamento
							if ( time === false ) {
								// se a opÃ§Ã£o de parar os ataques caso nÃ£o tenha tropas na aldeia
...
/** AUTO_BUILD - Adiciona ordens de construÃ§Ã£o em todas aldeias da vizualizaÃ§Ã£o de apenas uma vez */
var AUTO_BUILD = function() {
	...
	// coloca os levels atuais nos inputs de configuraÃ§Ã£o
	...
	// sistema de salve automatico (configuraÃ§Ã£o)
	...
	if ( document.getElementById( 'overview' ) && document.getElementById( 'overview' ).value === 'buildings' ) {
		...
		function BUILD( building, destroy ) {
			...
			// loop em todas aldeias da vizualizaÃ§Ã£o
			jQuery( '#buildings_table tr:not(:first)' ).each(function() {
				...
				if ( destroy ? data.can_destroy : data.buildings[ building ] ) {
					// quantidade total de ordens que jÃ¡ estÃ£o em andamento na aldeia
					...
					// quantidade de ordens do edificio a ser construido que jÃ¡ estÃ£o em andamento na aldeia
						orders = jQuery( 'td:last li:has(.order-status-light[style*=' + type + '])', this ).length;
					
					// executa quantas vezes for preciso atÃ© exercer o limite de ordens
...
/** CONFIGS - Janela de configuraÃ§Ãµes do script */
var CONFIGS = function() {
	// template com cada tipo de configuraÃ§Ã£o
	var configs = '';
	// template com as opÃ§Ãµes de linguagens disponiveis
	var langOptions = '';
	...
	// loop em cada tipo de configuraÃ§Ã£o
	jQuery.each([{
	...
	// quando alterado qualquer valor das configuraÃ§Ãµes
	app.find( '.TWA_CONFIGS_VALUE :input' ).change(function() {
		// salva os dados no cache
		DATA.configs[ this.name ] = this.value;
		localStorage[ 'TWA DATA' ] = JSON.stringify( DATA );
		
		// caso a configuraÃ§Ã£o alterada seja a linguagem
		// emite uma mensagem avisando que sÃ³ surtirÃ¡ efeito
		// na proxima execuÃ§Ã£o do script
		if ( this.name === 'lang' ) {
...
/** Faz a interaÃ§Ã£o entre usuÃ¡rio e cÃ³digo */
var INTERFACE = function() {
	...
	// caso o botÃ£o "Esc" seja pressionado fexa a janela
	...
	var APP = function( id, html, templ ) {
		...
		HTML( menu, '<li><a href="#">Â» ' + id + '</a></li>' ).click(function() {
...
/** Da inicio a execuÃ§Ã£o do script apos carregar dependencias */
var READY = function( callback ) {
	...
	// caso nÃ£o tenha nenhum dado no cache
	if ( jQuery.isEmptyObject( DATA.units ) || DATA.rev < rev ) {
		...
		// Ã© gerado novos dados apartir do servidor
		jQuery.when( getConfig, getUnits, getMail, getTranslates, getTemplates, getStyles ).then(function( config, units, dataMail, dataTranslates, dataTemplates, dataStyles ) {
			...
			// loop em todas configuraÃ§Ãµes do jogo
			...
			SET_LANG(function() {
				...
				// Ã© chamado a funÃ§Ã£o quando estiver tudo carregado
				callback();
			});
		});
	} else {
		...
		// Ã© pego o url atual para o funcionamento da funÃ§Ã£o GET_VILLAGE_INFO
...
/** Mostra ao jogador a opÃ§Ã£o para escolher a linguagem desejada */
 ...
 * str {String} - String que serÃ¡ formatada.
 * args {Object} - Objeto com os dados de substuiÃ§Ã£o dos templates. */
...
/** GET_VILLAGE_INFO - Obtem informaÃ§Ãµes bÃ¡sicas de uma aldeia apartir de coordenadas.
 * coords {String} - Coordenadas da aldeia que serÃ¡ obtida as informaÃ§Ãµes.
 * callback {Function} - FunÃ§Ã£o de retorno quando as informaÃ§Ãµes forem obtidas.
 */
var GET_VILLAGE_INFO = function( coords, callback, extras ) {
	...
	// caso a aldeia jÃ¡ esteja no cache, Ã© carregado rapidamente
...
var GET_PLAYER_INFO = function( name, callback, extras, id /* internal */ ) {
	...
	// caso o jogador jÃ¡ esteja no cache, Ã© carregado rapidamente
...
/** GET_VILLAGE_UNITS - Pega a quantidade maxima de tropas presentes em uma aldeia prÃ³pria.
...
var GET_VILLAGE_UNITS = function( data, callback, errorCallback ) {
	...
	// caso seja passado as coordenadas da aldeia, Ã© pego o ID e chamado
	// a funÃ§Ã£o novamente
	if ( isNaN( data ) ) {
		if ( data.length > 8 ) {
	...
	// carrega a pÃ¡gina da praÃ§a de reuniÃ£o para obter todas unidades da aldeia
...
var GET_COMMAND_TIME = function( data, callback ) {
	...
	function complete( html ) {
		...
		// filtra os comandos que estÃ£o retornando
	...
	// carrega a pÃ¡gina com os comandos em andamento
	return jQuery.getJSON( URL( 'overview&_partial', data ), function( data ) {
		complete( data.content );
	});
};

/** CHECK_OWN_VILLAGE - Verifica se a aldeia em questÃ£o Ã© do jogador da conta.
 * vid {Number} - ID da aldeia.
 * success {Function} - Caso a aldeia seja do jogador da conta serÃ¡ chamado esta funÃ§Ã£o.
 * error {Function} - Caso a aldeia nÃ£o seja do jogador da conta serÃ¡ chamado esta funÃ§Ã£o. */
 ...
 * content {Function|String|Number} - ConteÃºdo que serÃ¡ exibido no tooltip. Caso seja Function serÃ¡ mostrado o conteudo retornado da funÃ§Ã£o. */
...
/** CONFIRM - Gera uma tela de confirmaÃ§Ã£o.
 * text {String} - Texto que serÃ¡ exibido na confirmaÃ§Ã£o.
 * buttons {Array} - BotÃµes que serÃ£o exibidos na tela de confirmaÃ§Ã£o. */
...
/** DEFAULTS - Obtem os dados salvos no cache, caso nÃ£o esteja salvo retorna os dados padrÃµes de acordo com cada item.
 * name {String} - Nome do item que serÃ¡ obtido os dados salvos. */
